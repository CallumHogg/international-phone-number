...
(function() {
	...
	angular.module("internationalPhoneNumber", []).directive('internationalPhoneNumber', [
		'$timeout', function($timeout) {
			...
			//Set value
			$timeout(function(){;
			  	if (scope.ngModel.number){
			  		element.intlTelInput('setNumber', scope.ngModel.number);
			  	} else {
			  		element.intlTelInput('setNumber', scope.ngModel);
			  	}
			  	if (!scope.ngModel.config) scope.ngModel.config = $(element).intlTelInput("getSelectedCountryData");
			  	if (!scope.ngModel.type) scope.ngModel.type = $(element).intlTelInput("getNumberType");
			...
			read = function() {
				...
				scope.ngModel = {
					number: element.val(),
					config: $(element).intlTelInput("getSelectedCountryData")
				};
				return ctrl.$setViewValue(element.val());
			};
